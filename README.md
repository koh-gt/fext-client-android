...
1. Open the repository's **Releases** page (or the **`v8.0.1`** tag).
2. Download `fext-8.0.1-*-release.apk` from the release assets.
...
buildozer android debug          # produces bin/fext-8.0.1-*-debug.apk
```
...
```bash
adb install -r fext-8.0.1-*-release.apk
```
